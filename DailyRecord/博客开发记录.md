...
SourceTree并没有识别到我的文件,经过查询是因为文件夹下生成的.gitignore忽略文件配置里是写的 * 也就是忽略所有
我尝试将配置文件中的 * 改为.idea/ 后,就可以识别到我文件夹下的文件了,然后也忽略掉了IDEA环境下的额外文件
...
终于解决了当初实习困扰我的表格正好铺满页面不出现滚动条的问题(*^▽^*)：
...
***你给我通俗易懂、详细的解释一下符号链接、目录类型的符号链接、statSync和lstatSync的区别和应用场景，并写一个通俗易懂的总结提供给我用于记录。***
AI回答:
# 符号链接与文件系统检查方法详解
...
***那按照现在我需要给vitepress写一个自动路由，避免config里重复操作编写md路径，那用哪个比较好？无论是健壮性还是安全性的角度考虑。***
...
***用params.length和let file in params不都是一样的吗？***
...
***为什么我切换到/DailyRecord/这个页面是404错误呢？***
AI回答:
